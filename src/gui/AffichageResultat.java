    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int gx = 0; gx < gridSize; gx++) {
            for (int gy = 0; gy < gridSize; gy++) {
                ...
                g2.setColor(color);
                int x = margin + (gx * plotSize) / gridSize;
                int y = margin + (gy * plotSize) / gridSize;
                int nextX = margin + ((gx + 1) * plotSize) / gridSize;
                int nextY = margin + ((gy + 1) * plotSize) / gridSize;
                int currentCellWidth = nextX - x;
                int currentCellHeight = nextY - y;
